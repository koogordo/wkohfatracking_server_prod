...
DashboardController.get(
    ...
    (req: Request, res: Response) => {
        ...
        if (username) {
            userDB = req.params.id.split(':')[1].toLowerCase();
        } else {
            userDB = null;
        }
        ...
        if (username && userDB) {
            ...
            res.json(
                new Error(`user ${username} has no dash data, user not found.`)
            );
        }
    }
);
...
DashboardController.get(
    ...
    (req: Request, res: Response) => {
        ...
        if (username) {
            ...
            res.status(400).json({
                err: `user ${username} has no dash data, user not found.`,
            });
        }
    }
);
