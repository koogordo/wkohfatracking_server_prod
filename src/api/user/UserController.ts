...
UserController.post("/register", [checkJwt, checkRole("ADMIN")], (req: Request, res: Response) => {
    ...
    dao.users().find(req.body._id).then(user => {
...
UserController.post('/assignfamilies', [checkJwt], (req: Request, res: Response)=> {
    if (!req.body) {
        res.status(400).json({err: "Invalid request body", body: req.body})
    }
    const assignmentEditor = new ClientAssignmentEditor(dao);
    assignmentEditor.verifyFamilies(req.body.families).then(verification => {
        assignmentEditor.assignFamilies(verification.valid, req.body.userDBName).then((result: any) => {
...
UserController.post('/deassignfamilies', [checkJwt], (req: Request, res: Response) => {
    if (!req.body) {
        res.status(400).json({err: "Invalid request body", body: req.body})
    }
    const assignmentEditor = new ClientAssignmentEditor(dao);
    assignmentEditor.verifyFamilies(req.body.families).then(verification => {
        assignmentEditor.deassignFamilies(verification.valid, req.body.userDBName).then((result: any) => {
            result.valid = verification.valid;
            result.invalid = verification.invalid;
            if (result.success) {
                res.status(200).json(result);
