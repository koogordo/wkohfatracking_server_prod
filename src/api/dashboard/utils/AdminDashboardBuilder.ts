                ...
                .then((queuedVisitsArr: any[]) => {
                    ...
                    for (const visitArr of queuedVisitsArr) {
                        if (!visitArr.error && visitArr.length > 0) {
                            const visitsForDisplay = visitArr.map(
                                (doc: any) => {
                                    ...
                                    if (FormUtil.isCompressed(doc)) {
                                        ...
                                        visitDateQ = FormUtil.findFormPartByIndex(
                                            doc.form,
                                            visDateIndex
                                        );
                                        doc.form.visitDate = moment(
                                    ...
                                    doc.form.dateSubmitted = moment(
                                        doc.form.status[
                                            doc.form.status.length - 1
                                        ].date
                                    ).format('MMM DD YYYY');
                                    const firstOpenStatus = doc.form.status.find(
                                        (status: any) => {
                            ...
                            queuedVisits = queuedVisits.concat(
                                visitsForDisplay
                            );
                        }
                    }
